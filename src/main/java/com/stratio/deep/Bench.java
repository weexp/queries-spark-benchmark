...
public class Bench {
    ...
    public List<MetricValue> disk() {
        ...
        ReturnValue check_disk_all = null; // "check_diks_all" --> operaciones
                                           // del agent.ini
                                           // (/home/su/agentServer-0.1.0-alpha/conf)
        try {
            check_disk_all = jnrpeClient.sendCommand("check_disk_all");
        } catch (JNRPEClientException e) {
            e.printStackTrace();
        }
        Metric metric = MetricParser.generateResult(
                check_disk_all.getMessage(), check_disk_all.getStatus()
                        .getSeverity());
        List<MetricValue> list = metric.getMetricValues();// Sacar la operacion
                                                          // que interese
        return list;
    ...
    public List<MetricValue> cpu() {
        ...
        Metric metric = MetricParser.generateResult(
                check_cpu_load.getMessage(), check_cpu_load.getStatus()
                        .getSeverity());
        List<MetricValue> list = metric.getMetricValues();// Sacar la operacion
                                                          // que interese
    ...
    public List<MetricValue> cpu_avg() {
        ...
        Metric metric = MetricParser.generateResult(check_load_average
                .getMessage(), check_load_average.getStatus().getSeverity());
        List<MetricValue> list = metric.getMetricValues();// Sacar la operacion
                                                          // que interese
    ...
    public List<MetricValue> mem_free() throws JNRPEClientException {
        ...
        Metric metric = MetricParser.generateResult(
                check_mem_free.getMessage(), check_mem_free.getStatus()
                        .getSeverity());
        List<MetricValue> list = metric.getMetricValues();// Sacar la operacion
                                                          // que interese
    ...
    public List<MetricValue> mem_swap() throws JNRPEClientException {
        ...
        Metric metric = MetricParser.generateResult(
                check_mem_swap.getMessage(), check_mem_swap.getStatus()
                        .getSeverity());
        List<MetricValue> list = metric.getMetricValues();// Sacar la operacion
                                                          // que interese
    ...
    public List<MetricValue> disk_all() throws JNRPEClientException {
        ...
        ReturnValue check_disk_stats_all = jnrpeClient
                .sendCommand("check_disk_stats_all"); // "check_diks_all" -->
                                                      // operaciones del
                                                      // agent.ini
                                                      // (/home/su/agentServer-0.1.0-alpha/conf)
        Metric metric = MetricParser.generateResult(check_disk_stats_all
                .getMessage(), check_disk_stats_all.getStatus().getSeverity());
        List<MetricValue> list = metric.getMetricValues();// Sacar la operacion
                                                          // que interese
    ...
    /*
     * for (int i=0; i==9; i++) System.out.println(data[i]);
     * 
     * try { FileWriter fichero = new
     * FileWriter("C:\\Users\\ParadigmaTecnologico\\prueba.txt");
     * 
     * fichero.write(
     * "|*******************************************************************************************************|"
     * + "\r\n"); fichero.write(
     ...
     * "                                         |     ");// | | | |" + "\r\n");
     * fichero.write(tT + "      |       "); fichero.write(tT +
     * "       |       "); fichero.write(tT + "       |       ");
     * fichero.write(tT + "       |       ");
     * 
     * fichero.close();
